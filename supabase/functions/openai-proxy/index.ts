...
// This function securely proxies requests to OpenAI API without exposing the API key to the client
...
interface OpenAIRequest {
  type: 'scan-receipt' | 'financial-advice';
  imageData?: string;
  prompt?: string;
  groups?: any[];
  userId?: string;
...
serve(async (req) => {
  ...
  try {
    ...
    // Verify Supabase auth (optional but recommended)
    ...
    // Parse request body
    const { type, imageData, prompt, groups, userId }: OpenAIRequest = await req.json()
...
async function scanReceipt(apiKey: string, imageData: string) {
...
async function getFinancialAdvice(apiKey: string, groups: any[], userId: string) {
